...
from projector_method2 import Projector2
from Q1initialize import Q1_init
...
def build_mlp(
    ...
    with tf.variable_scope(scope):
        ...
        hidden_layer = tf.layers.dense(inputs=sy_input,
                                        ...
                                        kernel_initializer=tf.truncated_normal_initializer(stddev=0.0001, dtype=tf.float32), 
...
def train_PG(
    ...
    Aphi = 0
    Bphi = 1
    # the Lyapunov matrix from the stability analysis paper
    P0 = np.array([[0.0012,  0.0035,  0.0044,  0.0025],
                   [0.0035,  0.0218, -0.0639,  0.0112],
                   [0.0044, -0.0639,  2.4457,  0.0775],
                   [0.0025,  0.0112,  0.0775,  0.0233]
    ])
    Q10 = scipy.linalg.block_diag(np.linalg.inv(P0), np.eye(states_size))

    # newer method
    Q1initializer = Q1_init(env.AG, env.BG, env.CG)
    Q1init = Q1initializer.compute()
    Q10 = scipy.linalg.block_diag(Q1init, np.eye(states_size - env.nx))
    projector = Projector2(states_size, size, env.AG, env.BG, env.CG, Aphi, Bphi, Q10)
    # TODO: Use a Lyapunov initialized P (Q1inverse)
    ...
    for itr in range(n_iter):
        ...
        projector.updateRNN(agent.rnn, agent.sess)
...
def main():
    ...
    parser.add_argument('--exp_name', type=str, default='vehicle_partial_norm_proj_tilde')
